...
const DEFAULT_SPONSOR_IMAGES: Record<string, string> = {
...
interface SponsorsSectionProps {
  ...
  sponsorImages?: Record<string, string>;
  images?: string[];
}

export const SponsorsSection: React.FC<SponsorsSectionProps> = ({ driveFiles = [], sponsorImages = {}, images = [] }) => {
  ...
  const getSponsorLogoUrl = (sponsor: SponsorItem): string | null => {
    ...
    // 2. Check mapped local partner image
    if (sponsorImages[sponsor.id]) {
      return sponsorImages[sponsor.id];
    }
    // 3. Check default per-sponsor image mapping
    if (DEFAULT_SPONSOR_IMAGES[sponsor.id]) {
      return DEFAULT_SPONSOR_IMAGES[sponsor.id];
    }
    // 4. Check array-based image mapping by sponsor order
    const sponsorIndex = INITIAL_SPONSORS.findIndex((item) => item.id === sponsor.id);
    if (sponsorIndex >= 0 && images[sponsorIndex]) {
      return images[sponsorIndex];
    }
    // 5. Check matched Google Drive image
    const driveUrl = getDriveImageUrl(sponsor);
    if (driveUrl) {
      return driveUrl;
    }
    // 6. Default logo if available
