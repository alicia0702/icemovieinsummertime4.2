...
import { Camera, HardDrive, Image as ImageIcon, Sparkles, X, ExternalLink, Upload } from 'lucide-react';
...
interface CustomPhoto {
  ...
  name: string;
  url: string;
  source: 'drive' | 'upload' | 'gallery';
  date?: string;
}

export const PhotoGallerySection: React.FC<PhotoGallerySectionProps> = ({ driveFiles = [], images = [] }) => {
  const [uploadedPhotos, setUploadedPhotos] = useState<CustomPhoto[]>(() => {
    try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('icemovie_uploaded_gallery_photos', JSON.stringify(uploadedPhotos));
    } catch {
  ...
  const handlePhotoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Promise.all(
      imageFiles.map(
        (file) =>
          new Promise<CustomPhoto>((resolve, reject) => {
  ...
  const galleryPhotos: CustomPhoto[] = images.map((image, index) => ({
    id: `gallery-${index}`,
    name: `Foto ${index + 1}`,
    url: image,
  ...
  const allPhotos = [...galleryPhotos, ...drivePhotos, ...uploadedPhotos];
    ...
    <div className="bg-white border-2 border-[#1D9999]/30 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6 overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2">
          <label className="inline-flex items-center gap-1.5 text-xs font-extrabold text-[#1D9999] bg-[#1D9999]/10 hover:bg-[#1D9999] hover:text-white px-3 py-1.5 rounded-xl transition-all cursor-pointer border border-[#1D9999]/20 shadow-sm">
            <Upload className="w-3.5 h-3.5" />
            <span>Adaugă poze locale</span>
        ...
        <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
              Sincronizează fișierele Google Drive sau adaugă poze locale de pe computer pentru a completa galeria.
            </p>
          </div>
        </div>
      ) : (
            ...
            >
              ...
              <div className="absolute top-2 left-2 z-10">
                {photo.source === 'drive' ? (
                  ...
                  <span className="bg-slate-900/80 backdrop-blur-md text-[#1D9999] text-[10px] font-bold px-2 py-0.5 rounded-md border border-[#1D9999]/30 flex items-center gap-1 shadow">
